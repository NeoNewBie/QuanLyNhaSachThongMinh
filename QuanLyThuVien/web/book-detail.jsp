...
<body>
            ...
            <div class="col-md-7">
                <div class="d-flex justify-content-between align-items-center">
                    <div>
                        ...
                        <%-- 🛑 CHECK STOCK --%>
                        <c:choose>
                            <c:when test="${detail.stock > 0}"><span class="badge bg-light text-dark border ms-2 rounded-pill px-3">Còn hàng: ${detail.stock}</span></c:when>
                            <c:otherwise><span class="badge bg-danger ms-2 rounded-pill px-3 text-white">HẾT HÀNG</span></c:otherwise>
                        </c:choose>
                ...
                <div class="d-flex flex-wrap gap-3 mb-5">
                    <c:choose>
                        ...
                        <c:otherwise>
                            ...
                            <c:if test="${detail.stock <= 0}"><button class="btn btn-secondary btn-custom w-100 py-3" disabled>SẢN PHẨM HIỆN ĐANG HẾT HÀNG</button></c:if>
                        </c:otherwise>
                    </c:choose>
                </div>
                ...
                <%-- 🛑 KHÔI PHỤC PHẦN ĐÁNH GIÁ TÂM ĐẮC CỦA SẾP --%>
                <div class="container mt-5 mb-5">
                                ...
                                <c:forEach items="${listR}" var="r">
                                    <div class="py-3 border-bottom">
                                        <div class="d-flex align-items-start">
                                            <div class="me-3" style="width: 45px; height: 45px; background-color: #ED553B; color: white; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-weight: bold;">${r.username.substring(0,1)}</div>
                    ...
                    <div class="p-4 bg-light rounded-3 border">
                        ...
                        <form action="add-review" method="POST">
                            ...
                            <div class="mb-3">
                                ...
                                <select name="txt_so_sao" class="form-select border-0 shadow-sm"><option value="5">⭐⭐⭐⭐⭐ (Tuyệt vời)</option><option value="4">⭐⭐⭐⭐ (Rất hay)</option><option value="3">⭐⭐⭐ (Bình thường)</option><option value="2">⭐⭐ (Hơi tệ)</option><option value="1">⭐ (Rất tệ)</option></select>
                            </div>
                            <div class="mb-3"><textarea name="txt_binh_luan" class="form-control border-0 shadow-sm" rows="3" placeholder="Chia sẻ cảm nhận..." required></textarea></div>
    ...
    <%-- 🛑 KHÔI PHỤC MODAL MƯỢN SÁCH --%>
    <div class="modal fade" id="borrowModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content border-0 shadow-lg" style="border-radius: 25px;">
                <div class="modal-header bg-primary text-white py-3 border-0"><h5 class="modal-title fw-bold mx-auto">XÁC NHẬN MƯỢN SÁCH</h5></div>
                <form action="borrow" method="POST">
                    <div class="modal-body p-4 text-center">
                        ...
                        <div class="alert alert-info small text-start rounded-4">Thời hạn mượn mặc định là 14 ngày. Hãy trả sách đúng hạn nhé Hải!</div>
                    ...
                    <div class="modal-footer border-0 p-4 pt-0"><button type="submit" class="btn btn-primary w-100 rounded-pill fw-bold py-3 shadow">GỬI YÊU CẦU MƯỢN</button></div>
                </form>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function addToCartWithQty(id) {
            ...
            fetch('add-to-cart-async?id=' + id + '&quantity=' + qty).then(res => res.text()).then(data => {
                document.getElementById('cart-badge').innerText = data;
                Swal.fire({ icon: 'success', title: 'Đã thêm ' + qty + ' cuốn!', toast: true, position: 'top-end', timer: 1500, showConfirmButton: false });
            });
        }
        function buyNow(id) {
            const qty = document.getElementById('detail-qty').value;
